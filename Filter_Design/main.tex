...
\author{Sindam Ananya\\EE23BTECH11058}
\date{}
...
\begin{align}
    ...
    j&=13
\end{align}
 substituting $j =13$ gives the passband
range for our bandpass filter as $11.8$ kHz - $13.0$ kHz.  Hence, the un-normalized discrete time filter
passband frequencies are $F_{p1} = 11.8$ kHz
and $F_{p2} = 13.0$ kHz. \\
The corresponding normalized digital filter passband frequencies are
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.49 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.54 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 11.8-0.3 = 11.5 \text{KHz}\\
    F	_{s2} &= 13.0+0.3 = 13.3  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.479 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.554 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.968$, $\Omega_{p2} = 1.133$ and $\Omega_{s1} = 0.935$, $\Omega_{s2} = 1.185$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 1.047$ and $B = \Omega_{p2} - \Omega_{p1} = 0.165$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.441\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.571
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.441.
\end{align}
...
we get $N\geq 4$ and $0.349 \leq \epsilon \leq 0.61$\\
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/AnanyaSindam/EE1205/blob/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.349  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/AnanyaSindam/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/AnanyaSindam/EE1205/blob/main/Filter_Design/codes/pole_zero_plt.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.312$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.312}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_8}}\\
     &= \frac{0.312}{s_{L}^4 + 1.1068s_{L}^3 +1.6124s_{L}^2 + 0.959s_{L} +0.3365}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{2.48\times 10^{-4}s^4}{s^8 + 0.189s^7 + 4.598s^6 + 0.627s^5 + 7.581s^4 + 0.687s^3 + 5.521s^2 + 0.249s + 1.498} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{21.95 + 8.156z^{-1} + 83.6z^{-2} + 24.676z^{-3} + 119.156z^{-4} + 23.716z^{-5} + 76.592z^{-6} + 7.196z^{-7} + 18.446z^{-8}}
\end{align}

where $G=2.48\times 10^{-4}$    
\begin{figure}[H]
\centering
...
\end{align}
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= \brak{2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}}{n\pi}\\
            &= \frac{2\cos{\brak{0.515n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.515n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
